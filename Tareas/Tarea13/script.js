...
function editar(id) {
  ...
  fetch(url)
    ...
    .then((data) => {
		  ...
		  let modalBootstrap = new bootstrap.Modal(document.getElementById('myModal'));
modalBootstrap.show();
...
function renderizarTablaRead(objeto) {
  ...
  for (var i = 0; i < lista.length; i++) {
    html += `<tr>
        ...
        <td><a href="javascript:editar('${lista[i].id} ')">Editar</a>  <a href="javascript:abrirModalEliminar('${lista[i].id}')">Eliminar</a> </td>
...
function confirmarEliminar() {
  ...
  // Usamos fetch para hacer la petición a delete.php (o como se llame tu archivo PHP)
  fetch('delete.php', {
    method: 'POST',
  ...
  .then(data => {
    alert(data);
    cerrarModalEliminar();
    listar(); // Recarga la tabla si tienes una función listar()
  })
  .catch(error => {
